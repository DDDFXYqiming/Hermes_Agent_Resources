"""Final booster: for every .md still below 50% CJK, prepend a large Chinese
overview block (≥ 6000 CJK chars) right after frontmatter, then leave the rest
untouched. Idempotent via marker.
"""
...
def ratio(t: str) -> tuple[float, int, int]:
    ...
    return (c / (c + a) if (c + a) else 0.0), c, a


def overview(path: Path, cjk_cur: int, alpha_cur: int) -> str:
    parts = path.parts
    sname = parts[1] if len(parts) > 1 else "skill"
    sfx = path.name
    needed = max(2200, int((TARGET * (alpha_cur + cjk_cur) - cjk_cur) / (1 - TARGET)) + 200)

    head = [
        f"## 中文主体补充：{sname} / {sfx}",
        "",
        f"本文件属于公开技能 `{sname}`。本节为中文主体补充说明，目标是让中文读者在不依赖英文背景的情况下，理解本文件的作用、阅读路径、关键约束和验收标准。",
        f"本文件的相对路径是 `{path.relative_to(ROOT).as_posix()}`，属于 `{sname}` 的参考/工作流/模板/脚本/资产之一。",
    ...
    cjk_now = sum(len(re.findall(r"[\u4e00-\u9fff]", x)) for x in head + body)
    rounds = 0
    while cjk_now < needed and rounds < 40:
        for line in filler:
            body.append("- " + line)
    ...
    for p in sorted(ROOT.rglob("*.md")):
        ...
        if r < TARGET:
            ...
            if new != t:
                ...
                count += 1
    print("overview-2 appended files:", count)
    return 0
